...
function Chat(props) {
  ...
  const onChat = async () => {
    ...
    if (!roomId) {
      ...
      if (newRoomData && newRoomData.EC !== 0) {
        return;
      }
      console.log(newRoomData.DT);
      setRoomId(newRoomData.DT._id);
      localStorage.setItem("njs_asm3_roomId", newRoomData.DT._id);
    }
  };
  ...
  const handlerSend = async () => {
    ...
    let res = await ChatRoomsAPI.addMessage(data);
    if (res && res.EC === 0) {
      setTextMessage("");
            ...
            <div className="card card-bordered fix_boderChat" style={{ width: "fit-content" }}>
              ...
              <div className="ps-container ps-theme-default ps-active-y fix_scoll">
                {message &&
                  message.map((value) =>
                    !value.is_admin ? (
                      <div className="media media-chat media-chat-reverse" key={value._id}>
